...
def rst_to_latex(rst, top_heading_level=0):
    ...
    if rst is None:
        ...
        for heading_rst, heading, heading_level in _parse_rst_headings(tex):
            heading_level = min(
                heading_level + top_heading_level, max(heading_latex.keys())
            )
            tex = tex.replace(
                heading_rst, f"\\{heading_latex[heading_level]}{{{heading}}}"
            )
...
def text_box(string):
    ...
    new_string = (
        new_string.replace("\m \m", "\r").replace("\m\m", "\r").replace("\m", " ")
    )
    return new_string
...
def create_latex_pdf(
    ...
    def remove_temp_files(basename_):
        filenames = [f"{basename_}.tex", f"{basename_}.aux", f"{basename_}.log"]
...
def main():
    ...
    parser.add_argument(
        "filename", type=str, nargs="?", help="Python file with character definition"
    )
    parser.add_argument(
        "--editable",
        "-e",
